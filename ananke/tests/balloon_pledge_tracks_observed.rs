...
async fn pledge_grows_to_observed_peak() {
    ...
    h.observation.update_peak(&h.svc, mb(10 * 1024));
    step().await;
...
async fn pledge_clamps_to_max_on_overshoot() {
    ...
    h.observation.update_peak(&h.svc, mb(28 * 1024));
    step().await;
...
async fn pledge_decays_as_spike_rolls_out_of_window() {
    ...
    h.observation.update_peak(&h.svc, mb(12 * 1024));
    step().await;
    ...
    h.observation.update_peak(&h.svc, mb(4 * 1024));
    for _ in 0..7 {
        step().await;
    }
...
async fn pledge_does_not_emit_for_sub_threshold_drift() {
    ...
    h.observation.update_peak(&h.svc, mb(12 * 1024) + mb(50));
    step().await;
